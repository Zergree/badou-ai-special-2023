...
if __name__ == '__main__':
    ...
    '''
    axis=0 参数指定了要在哪个轴上增加维度。在这里，axis=0 表示在数组的第一个轴（即最外层的维度）上增加一个维度。
    这个操作的目的通常是为了将一个样本（例如一张图片）转换为一个批次（batch），以便输入到深度学习模型中进行处理。在深度学习中，通常会将输入数据组织成批次，每个批次包含多个样本。因此，为了与模型的输入要求匹配，可能需要对数据进行批次的转换和处理。
    具体到这个示例中，np.expand_dims(x, axis=0) 将原始的单个样本 x 增加了一个维度，使得它变成了一个包含单个样本的批次。转换后的形状可能是 (1, height, width, 3)，其中 height 和 width 是图像的尺寸，3 是通道数（假设是RGB图像）。
